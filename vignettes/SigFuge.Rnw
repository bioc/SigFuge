...
\subsection{Citation} For a more in depth discussion of clustering per-base expression profiles, see our manuscript describing the \texttt{SigFuge} algorithm in further detail.  Please try to cite the following article when you publish results obtained using the software.
\begin{quote}
...
Two objects are needed to run \texttt{SigFuge}: (1) a \texttt{GRanges} object or similar \texttt{data.frame} containing information about the locus (gene) of interest and (2) a data matrix of coverage for each sample at each base (position) across the locus. The \texttt{GRanges} object is only used to plot the exon boundaries, so it may be omitted if only calcualting \texttt{SigFuge} $p$-values.

There are two ways to create a \texttt{GRanges} object.  The first way is to manually input the start and end positions of exons, the chromosome, and the strand orientation.  If a gene has mutliple isoforms with overlapping exons, it is advised to take the union of all isoform models in order to detect possible alternative splicing.  Below is an example for manually creating a \texttt{GRanges} object containing annotation data for the \textit{CDKN2A} locus.
...
The second way to create a \texttt{GRanges} object is to extract the annotation information from an appropriate \texttt{R} package database.  Below shows how to extract gene annotation data from the set of UCSC known genes.  Notice that exons 4 and 5 are overlapping, so we use the \texttt{reduce()} function to merge these exons.
...
Now that we have the gene annotation, the next step is to construct the data matrix.  For each locus, a $d$-by-$n$ coverage matrix is required as input, where $d$ is the locus length and $n$ is the sample size.  Each column corresponds to a sample and each row to the coverage (number of overlapping reads) at each base.  We assume that the sequencing data has been aligned to a reference genome and the output is a sorted and indexed BAM file~\cite{samtools}.  To demonstrate how to extract coverage data from a list of BAM files, we will use the example BAM files contained in the \texttt{prebsdata} package (see~\cite{prebsdata} for the original data source).  First, we load, sort and index the BAM files for each sample.  We also create a \texttt{GRanges} object corresponding to the exons of gene DPM1 located on chromosome 20.
...
Next, we use the \texttt{GRanges} object to generate a matrix of per-base coverage. To do this, we first create a function to extract coverage, \texttt{calcInfo}.
...
It is important to note that the \texttt{GRanges} object is only used to define properties of the \texttt{SigFuge} figures and does not affect any of the analysis. Most notably, the locus information will not be used to parse the coverage matrix. As such, the matrix should only contain the exonic regions of interest and the matrix dimension $d$ should equal the total length of the exons specified in the \texttt{GRanges} object.


\section{\texttt{SigFuge} Case Study}
This section gives a fully worked case study.  We will analyze a subset of 179 lung squamous cell tumor samples sequenced as part of the Cancer Genome Atlas~\cite{TCGA}.  We will restrict attention to the \textit{CDKN2A} locus.  For this example, we skip the data retrieval step and apply \texttt{SigFuge} to a depth count matrix included with the package.  \texttt{geneDepth} is a data matrix of coverage with rows corresponding to genomic positions and columns to samples, and \texttt{geneAnnot} is a \texttt{GRanges} object containing annotation information about the \textit{CDKN2A} locus.
...
\subsection{\texttt{SFfigure}}
\texttt{SFfigure} is a comprehensive function that performs 4 main tasks: (1) normalization of the data matrix, (2) unsupervised clustering on the normalized values, (3) $p$-value calculation for significance of clustering, and (4) plotting of the data curves.  The following code uses the default normalization and clustering procedures to produce 3 plots (Figures~\ref{fig1}-\ref{fig3}).  These plots will be saved in the working directory as \texttt{CDKN2A\_1.pdf}, \texttt{CDKN2A\_2.pdf}, and \texttt{CDKN2A\_3.pdf}.  To speed up calculations, we will only consider the first 50 samples.
...
\subsection{\texttt{SFpval}}
\texttt{SFpval} allows you to obatin the $p$-value without creating any plots.  
...
This reported $p$-value may not perfectly match the $p$-value reported in the plots because the $p$-value calculation depends on simulation.  This function is useful if you are interested in testing hundreds or thousands of loci in the genome.  Once you have obtained all of the $p$-values, you can create plots for only the most significant genes.  Since \texttt{SFpval} does not correct for multiple comparisons, it is important to adjust the $p$-values using \texttt{p.adjust()} if iterating over multiple loci.

\subsection{\texttt{SFlabels} and \texttt{SFnormalize}}
Cluster labels can be computed with \texttt{SFlabels}.  First, we have to normalize the data using \texttt{SFnormalize}.  Because we are interested in alterations such as splicing and fusions instead of differential gene expression, the \texttt{SigFuge} normalization procedure adjusts for differences in total expression (read counts).  The normalization procedure first filters out lowly expressed samples because these samples are unable to provide any evidence of an alteration.

SFfigure <- function(data, locusname, annot = c(), flip.fig = 1, label.exon = 1,
					print.n = 1, data.labels = 0, label.colors = c(), flag = 1,
					lplots = 2, savestr = c(), titlestr = c(), pval = 1) {
    ...
    if (!is.null(savestr)) {
        ...
        if (savestr[2] == ".eps" || savestr[2] == ".ps") {
            cat(paste(".eps/.ps extensions do not support semi-transparency.", 
    ...
    if (length(data.labels) != n) {
        ...
        if (!all(data.labels == 0)) {
            cat(paste("data.labels input should be '0' or n-vector.",
    ...
    if (any(lplots > 1)) {
        ...
        if (!is.null(label.colors)) {
            if (nrow(label.colors) == K && ncol(label.colors) == 3) {
                ...
                cat(paste("label.colors must be Kx3 RGB matrix.",
        ...
        if (any(lplots == 2) || any(lplots == 5)) {
            ...
            if (any(lplots == 5)) {
                ...
                tplot <- tplot + 
                            ...
                            ggtitle(paste0(titlestr,", cluster medians"))
                if (!is.null(savestr)) {
        ...
        if (any(lplots == 4)) {
            ...
            if (is.null(savestr)) {
                cat("Plot option 4 can only be saved to file. Must provide savestr.")
            } else {

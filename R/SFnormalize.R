SFnormalize <- function(data, flag = 1) {
    ...
    if (sum(!flag) < 3) {
        stop('Too many lowly expressed samples to carry out normalization procedure.
             Must have at least 3 samples above threshold.
             More samples are recommended for reasonable inference from p-value.')
    }
  
    # normalize non-low expression samples
    data.norm <- data[,!flag]
    coverage <- apply(data.norm, 2, sum)
    data.norm <- apply(data.norm, 2, function(x) x/sum(x)*median(coverage))
